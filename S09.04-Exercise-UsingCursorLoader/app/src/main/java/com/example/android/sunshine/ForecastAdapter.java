...
//      TODO (7) Generate a weather summary with the date, description, high and low
        ...
        String date = SunshineDateUtils.getFriendlyDateString(mContext,dateLong,false);
        int weatherId = mCursor.getInt(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
        String desc = SunshineWeatherUtils.getStringForWeatherCondition(mContext,weatherId);
        double low = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        float high = mCursor.getFloat(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
        //String weatherForThisDay = mWeatherData[position];
//      TODO (8) Display the summary that you created above
        String highLow = SunshineWeatherUtils.formatHighLows(mContext,high,low);

        String summary = date + " - " + desc + " - " + highLow;
...
//      TODO (10) If mCursor is null, return 0. Otherwise, return the count of mCursor
     ...
     *
     * @param newCursor The new weather data source.
     */
